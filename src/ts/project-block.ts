const createSvg = (
  ...
  iconRect: SVGRectElement,
  iconLine: SVGLineElement,
...
const buttonStyles = (button: HTMLButtonElement): void => {
...
const nodeToArray = (
  node: HTMLCollectionOf<Element> | NodeListOf<ChildNode>
): Array<ChildNode> => {
...
const linkLogic = () => {
  ...
  nodeToArray(listOfTasks).map((task) =>
    nodeToArray(task.childNodes).map((item) => {
      ...
      const svgPath: string = "http://www.w3.org/2000/svg";
      ...
      if (
        !(iconSvg instanceof SVGSVGElement) ||
        !(iconRect instanceof SVGRectElement) ||
        !(iconLine instanceof SVGLineElement) ||
      ...
      item.addEventListener("mouseenter", () => {
        if (
          btnLinkParent[0] instanceof HTMLElement &&
          !(btnLinkParent[0].className === "button-href")
        ) {
...
const totalPointsStyle = (
  ...
  if (!headerOfProject || !headerOfProject.parentElement) {
    return;
  }

  const headerGroupOfButtons = headerOfProject.childNodes[1];

  if (!(headerGroupOfButtons instanceof HTMLElement)) {
    return;
  }
...
const totalPointsLogic = () => {
  ...
  const totalPointsSpanId: string = "#TOTAL_POINTS_SPAN_ID";
